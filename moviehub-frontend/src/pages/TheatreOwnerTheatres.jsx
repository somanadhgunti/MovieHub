...
import { Table, Button, Space, Drawer, Form, Input, InputNumber, Select, message, Card, Popconfirm, Spin, Alert, Typography, Tag } from "antd";
...
export const TheatreOwnerTheatres = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      if (editingTheatre) {
        ...
        notificationService.success("Theatre Updated", "Theatre details have been successfully saved.");
      } else {
        await createTheatre(values);
        notificationService.success("Theatre Created", "New theatre registered successfully.");
      }
      loadTheatres();
      handleClose();
    } catch (err) {
      console.error("Failed to save theatre:", err);
      notificationService.error("Failed to Save Theatre", err.response?.data?.message || "Failed to save theatre details.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      notificationService.success("Theatre Deleted", "Theatre record has been permanently deleted.");
      loadTheatres();
    } catch (err) {
      console.error("Failed to delete theatre:", err);
      notificationService.error("Deletion Failed", err.response?.data?.message || "Failed to delete theatre from database.");
    }
  };
...
export default TheatreOwnerTheatres;
